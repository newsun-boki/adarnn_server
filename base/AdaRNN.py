...
class AdaRNN(nn.Module):
    ...
    def forward_pre_train(self, x, len_win=0):
        out = self.gru_features(x) # x: batch*2(include domain s and t) len_seq feature
        fea = out[0] # batch*2 len_seq h_out
        if self.use_bottleneck == True:
            fea_bottleneck = self.bottleneck(fea[:, -1, :])#batch*2 1 bottleneck_width
            fc_out = self.fc(fea_bottleneck).squeeze() #batch*2 n_output
        else:
            fc_out = self.fc_out(fea[:, -1, :]).squeeze() #batch*2 n_output
        # out_weight_list = numlayer len_seq
        out_list_all, out_weight_list = out[1], out[2] # out_list_all: numlayer batch*2 len_seq feature
        out_list_s, out_list_t = self.get_features(out_list_all)#out_list_s: numlayer batch len_seq feature
        ...
        for i in range(len(out_list_s)):
            ...
            h_start = 0 
            for j in range(h_start, self.len_seq, 1):#window style
    ...
    def process_gate_weight(self, out, index):
        x_s = out[0: int(out.shape[0]//2)] # batch len_seq h_out 
        x_t = out[out.shape[0]//2: out.shape[0]]
        x_all = torch.cat((x_s, x_t), 2) # batch len_seq h_out*2
        x_all = x_all.view(x_all.shape[0], -1) # batch len_seq*h_out*2
        weight = torch.sigmoid(self.bn_lst[index](
            self.gate[index](x_all.float()))) # batch len_seq
        weight = torch.mean(weight, dim=0) # 1 len_seq
        res = self.softmax(weight).squeeze()# len_seq
        return res
    ...
    def forward_Boosting(self, x, weight_mat=None):
        ...
        out_list_all = out[1] # numlayer batch*2 len_seq feature
