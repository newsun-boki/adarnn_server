...
def TDC(num_domain, data_file, station, dis_type = 'coral'):
    ...
    feat_shape_1 = feat.shape[1] #猜测应该是每天有24个小时
    feat =feat.reshape(-1, feat.shape[2])#把日和时整合到一起
    feat = feat.cuda()
            ...
            for can in candidate:
                ...
                for i in range(1, len(selected)-1):
                    for j in range(i, len(selected)-1):
                        index_part1_start = start + math.floor(selected[i-1] / split_N * num_day) * feat_shape_1#part1开始的小时
                        index_part1_end = start + math.floor(selected[i] / split_N * num_day) * feat_shape_1#part1结束的小时
