...
def train_AdaRNN(args, model, optimizer, train_loader_list, epoch, dist_old=None, weight_mat=None):
    ...
    for data_all in tqdm(zip(*train_loader_list), total=len_loader):#train_loader_list是每个temporal的数据的list
        optimizer.zero_grad()#注意这里应该是按照batch分段的
        list_feat = []#D_n(k段中属于第几段) batch len_seq feature
        list_label = []
        ...
        for temp_index in index:#判断多段之间是否一定相等，由于是按照batch分段的，并且batch_size比较小
            s1 = temp_index[0]
            s2 = temp_index[1]
        ...
        for i in range(len(index)):
            ...
            # feature_all: batch*2(include domain s and t) len_seq feature
            ...
            pred_s = pred_all[0:feature_s.size(0)]#batch n_output
            ...
            total_loss = total_loss + loss_s + loss_t + args.dw * loss_transfer # args.dw : \lamda
        loss_all.append(
